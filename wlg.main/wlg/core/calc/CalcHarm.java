...
import wlg.core.bean.zhanfa.YuanMenSheJi;
import wlg.core.bean.zhanfa.ZFType;
import wlg.core.bean.zhanfa.ZhanBiZhanFa;
import wlg.core.bean.zhanfa.ZhanFa;
import wlg.core.bean.zhanfa.ZhuGeJinNang;
...
public class CalcHarm {
	...
	public static <T extends ZhanFa> float calcKongZhiHuiHe(HuiHe huihe, boolean calcPrimy,List<ZhanFa> kongzhiZf,T[] zhanfas) {
		...
		for(int i=0;i<kongzhiZf.size();i++) {
			...
			//怯心夺志
			}else if(zf.getT().equals(ZFType.ZhuiJi_GongJi_FengFaShu)) {
				unHurtVal = calcQieXinDuoZhi(huihe,calcPrimy,kongzhiMap,zf,zhanfas);
			//谋主
			}else if(zf.getT().equals(ZFType.ZhuDong_YiChu_GuiBi_DongCha_XianShou)) {
				unHurtVal = calcMouZhu(huihe,calcPrimy,kongzhiMap,zf,zhanfas);
			//落雷
			}else if(zf.getT().equals(ZFType.ZhuDong_FaShu_HunLuan)) {
				unHurtVal = calcLuoLei(huihe,calcPrimy, kongzhiMap, zf, zhanfas);
			//诸葛锦囊
			...
			//辕门射戟
			}else if(zf.getT().equals(ZFType.ZhuDong_JianGongJi)) {
				unHurtVal = calcYuanMenSheJi(huihe,calcPrimy, kongzhiMap, zf, zhanfas);
			}
	...
	private static float calcYuanMenSheJi(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
			ZhanFa... zhanfas) {
		//控制战法发动成功的概率
		float rate = CalcDoRate.getKongZhiRate(huihe,zf);
		//不能发动战法时，直接返回
		...
		//减谋略属性值 前两回合封战法
		float fengVal = ((YuanMenSheJi)zf).getJianGJVal();
		
		float unHurtVal = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(fengVal),calcPrimy,zhanfas);
	...
	private static float calcZhuGeJinNang(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		//受谋略影响
		float val = zf.getStrategy()/Conf.shuxing_suoxiao;
		float fengVal = ((ZhuGeJinNang)zf).getJianshangVal() + val;
		
		float unHurtVal = kongzhiVal * ((ZhuGeJinNang)zf).getKeephuihe() * calcKongZhiAllHuiHe(huihe.getAllFeng(fengVal),calcPrimy,zhanfas);
		kongzhiMap.put(zf.getName(), kongzhiVal);
		return unHurtVal;
	}

	private static float calcLuoLei(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
			ZhanFa... zhanfas) {
		ZhanFa b = zf;
		float unHurtVal = 0.0f;
		//控制战法发动成功的概率
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getAllFeng(fengVal),calcPrimy,zhanfas);
	...
	private static float calcMouZhu(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
			ZhanFa... zhanfas) {
		MouZhu b = (MouZhu)zf;
		float unHurtVal = 0.0f;
		//控制战法发动成功的概率
		...
		for(int p:b.getPersons().getPersons()) {
			...
			//控制主的概率
			float kongzhiVal = rate*unHurt * b.getDoneRate();
			float tmp = kongzhiVal * b.getKeepHuihe() * calcKongZhiAllHuiHe(huihe.getGuiBi(b.getGuibiRate(), 0.0f),calcPrimy,zhanfas);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static float calcQieXinDuoZhi(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
			ZhanFa... zhanfas) {
		ZhanFa b = zf;
		float unHurtVal = 0.0f;
		//控制战法发动成功的概率
		...
		for(int p:b.getPersons().getPersons()) {
			...
			//控制主的概率
			float kongzhiVal = rate*unHurt * b.getDoneRate();
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengZhanfa(1.0f),calcPrimy,zhanfas);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
