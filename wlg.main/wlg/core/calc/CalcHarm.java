...
public class CalcHarm {
	...
	private static float calcMuYiFuMeng(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = 0.0f;
			if(huihe.getId()==1) {
				tmp =  kongzhiVal * calcKongZhiAllHuiHe(huihe.getGuiBi(0.5f,b.getJianshangVal()),calcPrimy,allZfs);
			} else {
				tmp =  kongzhiVal * calcKongZhiAllHuiHe(huihe.getAllFeng(b.getJianshangVal()),calcPrimy,allZfs);
			}
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
	...
	private static float calcJianShuXing(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:zf.getPersons().getPersons()) {
			...
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getAllFeng(huihe.getUpQuanShuXing(),true),calcPrimy,allZfs);
	...
	private static float calcZhuiJi(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(1.0f,p),calcPrimy,allZfs);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static float calcBiyue(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = fengAll * b.getKeephuihe() * calcKongZhiAllHuiHe(huihe.getAllFeng(b.getDoneRate()),calcPrimy,allZfs);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static float calcShiJi(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		float tmp = fengAll * calcKongZhiAllHuiHe(huihe.getAllFeng(b.getDownVal()),calcPrimy,allZfs);
		kongzhiMap.put(b.getName(), kongzhiVal);
		unHurtVal += tmp;
	...
	private static float calcJiaShuXing(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, 
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float fenggongji =  0.4f;
			float fengfashu =  0.4f;
	...
	private static <T extends ZhanFa> float calcFanjizhice(HuiHe huihe, boolean calcPrimy,Map<String, Float> kongzhiMap, ZhanFa zf, T... zhanfa) {
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = 0.0f;
			if(huihe.getId()==1) {
				tmp =  kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengZhanfa(1.0f),calcPrimy,zhanfa);
			}else {
				tmp =  kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengZhanfa(b.getHarmRate()),calcPrimy,zhanfa);
			}
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
	...
	private static <T extends ZhanFa> float calcJianshang(HuiHe huihe, boolean calcPrimy,Map<String, Float> kongzhiMap, ZhanFa zf, T... zhanfa) {
		...
		//控制主的概率
		float kongzhiVal = unHurt * b.getDoneRate();
		float unHurtVal = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(b.getExHarmVal(),p),calcPrimy,zhanfa);
		kongzhiMap.put(b.getName(), kongzhiVal);
		return unHurtVal;
	}
	...
	private static <T extends ZhanFa> float calcFashuShangHaiThenKZGongji(HuiHe huihe, boolean calcPrimy,Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:b.getPersons().getPersons()) {
			...
			//控制主的概率
			float kongzhiVal = unHurt * b.getDoneRate();
			float tmp = b.getKeephuihe() * kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(1.0f,p),calcPrimy,zhanfa);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static <T extends ZhanFa> float calcKZGongJiThenFaShuShanghai(HuiHe huihe,boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf,
		...
		for(int p:b.getPersons().getPersons()) {
			...
			//控制主的概率
			float kongzhiVal = unHurt * b.getDoneRate();
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(1.0f,p),calcPrimy,zhanfa);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static <T extends ZhanFa> float calcKZGongJi(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf, T... zhanfa) {
		...
		for(int p:b.getPersons().getPersons()) {
			...
			float tmp = kongzhiVal * calcKongZhiAllHuiHe(huihe.getFengGongji(b.getKonzhiVal(),p),calcPrimy,zhanfa);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
	...
	private static <T extends ZhanFa> float calcKZZhanFa(HuiHe huihe, boolean calcPrimy, Map<String, Float> kongzhiMap, ZhanFa zf, T... zhanfa) {
		...
		for(int p:b.getPersons().getPersons()) {
			...
			//控制主的概率
			float kongzhiVal = unHurt * b.getDoneRate();
			float tmp = kongzhiVal * b.getKeep() * calcKongZhiAllHuiHe(huihe.getAllFeng(b.getDoneRate()),calcPrimy,zhanfa);
			kongzhiMap.put(b.getName(), kongzhiVal);
			unHurtVal += tmp;
		}
		return unHurtVal;
